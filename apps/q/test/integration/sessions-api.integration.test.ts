...
import { type ConversationModel, IntentSchema, TurnSchema } from "@q/domain/conversation/model"
...
const texts = (model: ConversationModel) => model.messages.map((m) => m.text)
...
layer(HttpPlatformLayerTest, { excludeTestServices: true })("sessions API", (it) => {
  ...
  it.effect("escape from a second request ends the turn the first request is streaming", () =>
    Effect.gen(function* () {
      ...
      const held = Layer.succeed(
        AgentService,
        AgentService.of({ stream: () => Stream.concat(Stream.make("a"), Stream.fromEffect(Effect.as(Deferred.await(gate), "b"))) }),
      )
      const api = yield* makeApiClientIntegration(held)
      ...
      yield* Stream.runHead(
        Stream.filter(yield* api.sessions.watchSession({ params: { id: session.id } }), (m) => m.messages[1]?.text === "a"),
      )
  ...
  it.effect("a prompt while a turn is running is ignored; the stream still ends with the turn", () =>
    Effect.gen(function* () {
      ...
      const held = Layer.succeed(
        AgentService,
        AgentService.of({ stream: () => Stream.fromEffect(Effect.as(Deferred.await(gate), "done")) }),
      )
      const api = yield* makeApiClientIntegration(held)
