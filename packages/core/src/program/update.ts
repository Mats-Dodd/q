...
const applyEvent = (model: ConversationModel, event: ConversationEvent): ConversationModel =>
  ConversationEventSchema.match(event, {
    ...
    TurnEnded: ({ text }) => (model.turn._tag === "Streaming" ? endTurn(appendText(model, model.turn.messageId, text)) : model),
  })

// UPDATE

const beginStreaming = (model: ConversationModel, prompt: string): ConversationModel => {
  ...
  return Struct.evolve(model, {
    ...
    turn: () => TurnSchema.cases.Streaming.make({ messageId: assistantId, prompt }),
    nextId: (n) => n + 2,
  })
}

const appendText = (model: ConversationModel, messageId: number, chunk: string): ConversationModel =>
  Struct.evolve(model, {
...
const textOf = (model: ConversationModel, messageId: number): string =>
  model.messages.find((message) => message.id === messageId)?.text ?? ""

const endTurn = (model: ConversationModel): ConversationModel => Struct.evolve(model, { turn: () => TurnSchema.cases.Idle.make({}) })

const endTurnWithNotice = (model: ConversationModel, notice: string): ConversationModel =>
  Struct.evolve(model, { turn: () => TurnSchema.cases.Idle.make({}), notice: () => Option.some(notice) })
 ...
 */
const commit = (model: ConversationModel, messageId: number, outcome: Outcome): Return => ({
  model: endTurn(model),
  commands: [CommitTurn({ messageId, text: textOf(model, messageId), outcome })],
})

const isStreaming = (model: ConversationModel, messageId: number) => model.turn._tag === "Streaming" && model.turn.messageId === messageId
...
export const update = (model: ConversationModel, message: Message): Return =>
  MessageSchema.match(message, {
    ...
    SucceededAcceptPrompt: () => (model.turn._tag === "Accepting" ? { model: beginStreaming(model, model.turn.prompt) } : { model }),
    FailedAcceptPrompt: ({ error }) =>
      model.turn._tag === "Accepting" ? { model: endTurnWithNotice(model, `could not save prompt: ${error}`) } : { model },
    ReceivedText: ({ messageId, text }) => (isStreaming(model, messageId) ? { model: appendText(model, messageId, text) } : { model }),
    CompletedTurn: ({ messageId }) =>
      isStreaming(model, messageId) ? commit(model, messageId, OutcomeSchema.cases.Completed.make({})) : { model },
    FailedTurn: ({ messageId, error }) =>
      isStreaming(model, messageId)
        ? commit(appendText(model, messageId, ` [error: ${error}]`), messageId, OutcomeSchema.cases.Failed.make({ error }))
        : { model },
    PressedEscape: () =>
      model.turn._tag === "Streaming" ? commit(model, model.turn.messageId, OutcomeSchema.cases.Cancelled.make({})) : { model },
